...
function MobileFeatureListMenu() {
  ...
  return (
    <>
      <div className={cn('fixed inset-x-0 bottom-0 z-40 border-t border-gray-200 bg-white transition-transform duration-300 lg:hidden', isMobileMenuOpen ? 'translate-y-0' : 'translate-y-full')}>
      ...
      <button onClick={() => setIsMobileMenuOpen(!isMobileMenuOpen)} className='fixed bottom-6 right-6 z-50 rounded-xl border border-gray-200 bg-white p-3 shadow-lg lg:hidden'>
  ...
  return (
    <Link
      ...
      className={cn(
        ...
        isActive ? 'border-gray-200 bg-gray-50 text-gray-900 hover:bg-gray-50' : 'text-gray-700 hover:text-gray-900'
      )}
    >
      <div className='flex items-center gap-2'>
        <h3 className='text-sm font-medium leading-tight'>{title}</h3>
        {isPublic && <span className='inline-flex items-center rounded bg-green-50 px-1.5 py-0.5 text-xs font-medium text-green-700'>Public</span>}
      </div>
    </Link>
  );
}
